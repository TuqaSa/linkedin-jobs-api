...
Query.prototype.getJobs = async function () {
	...
	const MAX_CONSECUTIVE_ERRORS = 1000;  // Changed from 3 to 1000
	...
	try {
		...
		while (hasMore) {
			try {
				...
				// Add improved random delay between requests (between 3-7 seconds)
				const minDelay = 3000;
				const maxDelay = 7000;
...
Query.prototype.fetchJobBatch = async function (start) {
	...
	try {
		...
		if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
			console.log('Request timed out, waiting for 10 seconds before retrying...');
			await delay(10000); // Wait 10 seconds
			// Retry the request recursively
			return this.fetchJobBatch(start);
		}
		throw error;
	}
};
